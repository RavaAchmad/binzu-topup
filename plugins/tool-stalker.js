...
let handler = async (m, { conn, text, usedPrefix, command: cmd }) => {
    ...
    if (!(await conn.onWhatsApp(num))[0]?.exists) throw 'User tidak ditemukan'
    try {
...
handler.help = ['wastalk @tag/628xx']
handler.tags = ['tools']
