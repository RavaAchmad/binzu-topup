...
export default async function (m, conn = { user: {} }) {
  const senderJid = typeof m.sender === 'string' && m.sender ? m.sender : (m.key?.participant || m.key?.remoteJid || '0@s.whatsapp.net')
  let _name = await Promise.resolve(conn.getName(senderJid)).catch(() => '') || ''
  let sender = PhoneNumber('+' + senderJid.replace(/@.+/, '')).getNumber('international') + (_name ? ' ~' + _name : '')
  let chat = await Promise.resolve(conn.getName(m.chat)).catch(() => '') || ''
  // let ansi = '\x1b['
  let img
  try {
    if (global.opts['img'])
  ...
  console.log(`${chalk.red('——————————————— B  I N Z U ———————————————')}
...
`.trim(),
  ...
  if (m.messageStubParameters) console.log((await Promise.all(m.messageStubParameters.map(async jid => {
