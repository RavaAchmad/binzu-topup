...
// ✅ FIX: Helper aman untuk getWinner — kalau room ke-delete, return status null biar loop berhenti
const safeGetWinner = (id, data) => {
    try {
        const result = getWinner(id, data);
        if (!result) return { status: null, voting: false, _roomGone: true };
        return result;
    } catch {
        return { status: null, voting: false, _roomGone: true };
    }
};

// ✅ FIX: Helper aman untuk sesi — return null (bukan false) kalau ga ada
const safeSesi = (id, data) => {
    try {
        const result = sesi(id, data);
        return result || null;
    } catch {
        return null;
    }
...
// ✅ FIX: Tambah `return false` eksplisit di akhir — sebelumnya return undefined
const getPlayerById2 = (sender, id, data) => {
    ...
    return false; // ✅ explicit false, bukan undefined
};
...
const roleAmount = (from, data) => {
    ...
    if (result.player.length == 4) {
        return { werewolf: 1, seer: 1, guardian: 1, warga: 1, sorcere: 0 };
    } else if (result.player.length == 5) {
        return { werewolf: 1, seer: 1, guardian: 1, warga: 3, sorcere: 0 };
    } else if (result.player.length == 6) {
        return { werewolf: 2, seer: 1, guardian: 1, warga: 2, sorcere: 0 };
    } else if (result.player.length == 7) {
        return { werewolf: 2, seer: 1, guardian: 1, warga: 3, sorcere: 0 };
    } else if (result.player.length == 8) {
        return { werewolf: 2, seer: 1, guardian: 1, warga: 4, sorcere: 0 };
    } else if (result.player.length == 9) {
        return { werewolf: 2, seer: 1, guardian: 1, warga: 4, sorcere: 1 };
    } else if (result.player.length == 10) {
        return { werewolf: 2, seer: 1, guardian: 1, warga: 5, sorcere: 1 };
    } else if (result.player.length == 11) {
        return { werewolf: 2, seer: 1, guardian: 2, warga: 5, sorcere: 1 };
    } else if (result.player.length == 12) {
        return { werewolf: 2, seer: 1, guardian: 2, warga: 6, sorcere: 1 };
    } else if (result.player.length == 13) {
        return { werewolf: 2, seer: 1, guardian: 1, warga: 7, sorcere: 1 };
    } else if (result.player.length == 14) {
        return { werewolf: 2, seer: 2, guardian: 2, warga: 7, sorcere: 1 };
    } else if (result.player.length == 15) {
        return { werewolf: 3, seer: 2, guardian: 3, warga: 6, sorcere: 1 };
    }
};
...
// ✅ FIX: pagii — guard kalau data, data.dead, atau data.player undefined
const pagii = (data) => {
    if (!data || !data.dead || !data.player) return "*⌂ W E R E W O L F - G A M E*\n\n⚠️ Data tidak tersedia.";
    if (data.dead.length < 1) {
        ...
        for (let i = 0; i < data.dead.length; i++) {
            ...
            if (b === -1) continue; // ✅ skip kalau player ga ketemu
...
// ✅ FIX: pagi — guard kalau x null/false (room ke-delete pas sleep)
async function pagi(conn, x, data) {
    if (!x || !x.room || !x.player) return; // ✅ bail out aman
    try {
        ...
        console.error("[pagi error]", err);
    }
}

// ✅ FIX: voting — guard kalau x null/false
...
// ✅ FIX: malam — guard kalau x null/false
async function malam(conn, x, data) {
    ...
    try {
        ...
        if (hasil_vote === 0) {
            ...
            return conn
                ...
                .then(() => {
                    ...
                    const w = safeGetWinner(x.room, data);
                    if (w && w.status != null) return win(x, 1, conn, data);
                });
        ...
        console.error("[malam error]", err);
    }
}

// ✅ FIX: skill — guard kalau x null/false
async function skill(conn, x, data) {
    if (!x || !x.room || !x.player) return; // ✅ bail out aman
    try {
        skillOn(x.room, data);
        const w = safeGetWinner(x.room, data);
        if (!w || w.status != null || x.win != null) {
            ...
            for (let i = 0; i < x.player.length; i++) {
                if (x.player[i].role === "werewolf") {
                    if (x.player[i].isdead != true) {
                        ...
                        await conn.sendMessage(x.player[i].id, { text: textnya, mentions: membernya });
                    }
...
// ✅ FIX: win — guard kalau x null/false
async function win(x, t, conn, data) {
    if (!x || !x.room || !x.player) return; // ✅ bail out aman
    try {
        const sesinya = x.room;
        const winner = safeGetWinner(x.room, data);
        if (!winner) return; // room udah ga ada
        if (winner.status === false || x.iswin === false) {
...
// ✅ FIX: run — pakai safeGetWinner + cek sesi setelah setiap sleep
async function run(conn, id, data) {
    try {
        while (safeGetWinner(id, data).status === null) {
            await sleep(90000);

            let x = safeSesi(id, data);
            if (!x) break; // room hilang pas sleep, stop loop
            if (safeGetWinner(id, data).status !== null) { await win(x, 1, conn, data); break; }

            await pagi(conn, x, data);
            ...
            await malam(conn, x, data);

            x = safeSesi(id, data);
            if (!x) break;
            if (safeGetWinner(id, data).status !== null) { await win(x, 1, conn, data); break; }

            await skill(conn, x, data);

            x = safeSesi(id, data);
            if (!x || safeGetWinner(id, data).status !== null) break;
        }
        const finalRoom = safeSesi(id, data);
        if (finalRoom) await win(finalRoom, 1, conn, data);
    } catch (err) {
...
async function run_vote(conn, id, data) {
    try {
        ...
        console.error("[run_vote error]", err);
    }
}
...
async function run_malam(conn, id, data) {
    try {
        ...
        console.error("[run_malam error]", err);
    }
}
...
async function run_pagi(conn, id, data) {
    try {
        ...
        console.error("[run_pagi error]", err);
    }
}
